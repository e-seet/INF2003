...
const populateMockData = async () => {
    try {
		...
		// 2.	Insert into Category
		const categoryData = await readCSV(path.join(__dirname, 'csv_data', 'category.csv'));
		...
		// 6.	Insert into EventCategory
		  // to do
		  const eventCategoryData = await readCSV(path.join(__dirname, 'csv_data', 'event_category.csv'));
		...
		const uniqueEventCategoryData = mappedEventCategoryData.filter((value, index, self) =>
